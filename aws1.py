...
def log_to_db(user_id, task, model_family, model_name, input_length, output, error):
    ...
    try:
        ...
        # Note: In your original code, task_type and error_flag were used 
        # but only task and error were passed as arguments. Fixed here:
        cur.execute("""
            INSERT INTO inference_logs (
                user_id, task_type, model_family, model_name, input_length, output, error_flag
            ) VALUES (%s, %s, %s, %s, %s, %s, %s)
        """, (user_id, task, model_family, model_name, input_length, str(output), error))
        ...
        print(f"DB ERROR during logging: {e}")
        if conn:
